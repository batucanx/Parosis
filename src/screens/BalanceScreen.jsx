import { Plus, Check } from '../components/Icons.jsx'
  ...
  return (
    <div className="screen-in flex flex-col gap-5 px-5 pb-6 pt-2">
      ...
      <section className="mt-2">
        <h2 className="px-1 text-[17px] font-extrabold tracking-tight text-ink">Hesap Ekstresi</h2>

        <div className="glass mt-3.5 overflow-hidden rounded-[1.5rem]">
          {/* Başlık satırı */}
          <div className="grid grid-cols-[2.9rem_1fr_3.9rem_3.9rem] gap-x-1.5 border-b border-white/70 bg-white/35 px-3.5 py-3">
          ...
          {/* Satırlar — geniş satır aralığı ile */}
          <ul>
            {statement.map((row, i) => (
              <li
                key={row.id}
                className={`grid grid-cols-[2.9rem_1fr_3.9rem_3.9rem] items-center gap-x-1.5 px-3.5 py-4 ${
                  i > 0 ? 'border-t border-white/55' : ''
                }`}
              >
